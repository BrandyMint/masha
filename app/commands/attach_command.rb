...
class AttachCommand < BaseCommand
  ...
  def call(project_slug = nil, *)
    if project_slug.blank?
      ...
      message = if project.update(telegram_chat_id: chat['id'])
                  "Установили этот чат основным в проекте #{project}"
                else
                  "❌ Ошибка при присоединении чата: #{project.errors.full_messages.join(', ')}"
                end
    else
