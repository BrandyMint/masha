...
ActiveAdmin.register User do
  ...
  index do
    ...
    column :memberships do |user|
      ul do
        user.memberships.includes(:project).find_each do |m|
          li do
    ...
    column :authentications do |user|
      ul do
        user.authentications.each do |_a|
          li do
            link_to '{a.provider} ({a.nickname}) <{a.email}>',
                    admin_authentications_url(q: { user_id_in: user.id })
          end
        end
      end
    rescue StandardError => e
