...
RSpec.describe SummaryQuery do
  ...
  describe '#projects_to_users_matrix' do
    ...
    it 'includes project in matrix' do
      ...
      expect(result[:matrix][project][user]).to be_positive
    end
  ...
  describe '#list_by_days' do
    ...
    it 'includes daily totals' do
      ...
      expect(result[:total_by_date][today]).to be_positive
      expect(result[:total_by_date][today - 1.day]).to be_positive
    end

    it 'includes column totals' do
      result = query.list_by_days

      expect(result[:total_by_column][project.to_s]).to be_positive
    end

    it 'includes total hours' do
      result = query.list_by_days

      expect(result[:total]).to be_positive
    end
  end
end
