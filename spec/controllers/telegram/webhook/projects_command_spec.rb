...
RSpec.describe Telegram::WebhookController, telegram_bot: :rails, type: :telegram_bot_controller do
  ...
  context 'rename operations', :callback_query do
    ...
    # SKIP: Session не передается между dispatch() - требует ручного тестирования или переработки
    # context 'rename project slug only'
    # context 'rename project both title and slug'
  end
  ...
  context 'client management', :callback_query do
    ...
    # SKIP: Session не передается между dispatch() - требует ручного тестирования
    # it 'sets client for project'

    # SKIP: Session не передается между dispatch() - требует ручного тестирования
    # it 'removes client from project'
  end
  ...
  context 'delete project', :callback_query do
    ...
    before do
      # Ensure user has owner membership to the project
      memberships(:telegram_work)
      # Удаляем все invites чтобы избежать foreign key constraint
      project.invites.destroy_all
    end

    # SKIP: Session не передается между dispatch() - требует ручного тестирования
    # it 'deletes project through workflow'
    # it 'cancels deletion on wrong name'
  end
end
