  ...
  context 'authenticated user' do
    ...
    context 'complete adduser workflow with callback queries', :callback_query do
      ...
      it 'adds user through complete interactive workflow' do
        ...
        expect do
          dispatch_command :adduser, project1.slug, 'test_new_user', 'member'
        ...
        expect(new_membership.user.nickname).to eq('test_new_user')
      ...
      it 'adds user with owner role directly' do
        ...
        expect do
          dispatch_command :adduser, project1.slug, 'test_owner_user', 'owner'
        end.to change(Membership, :count).by(1)

        new_membership = Membership.last
        expect(new_membership.project).to eq(project1)
        expect(new_membership.user.nickname).to eq('test_owner_user')
      ...
      it 'adds user with viewer role directly' do
        ...
        expect do
          dispatch_command :adduser, project1.slug, 'test_viewer_user', 'viewer'
        end.to change(Membership, :count).by(1)

        new_membership = Membership.last
        expect(new_membership.project).to eq(project1)
        expect(new_membership.user.nickname).to eq('test_viewer_user')
      ...
      it 'handles username with @ symbol directly' do
        ...
        expect do
          dispatch_command :adduser, project1.slug, '@test_at_user', 'member'
        end.to change(Membership, :count).by(1)

        # Проверяем что пользователь добавлен
        new_membership = Membership.last
        expect(new_membership.project).to eq(project1)
        expect(new_membership.user.nickname).to eq('test_at_user')
        expect(new_membership.role_cd).to eq(2) # member role
      end
      ...
      it 'handles non-existent project directly' do
        ...
        expect do
          dispatch_command :adduser, 'nonexistent-project', 'test_nonexistent_user', 'member'
      ...
      it 'handles direct add with parameters' do
        ...
        expect do
          dispatch_command :adduser, project1.slug, 'test_final_user', 'member'
        end.to change(Membership, :count).by(1)

        new_membership = Membership.last
        expect(new_membership.project).to eq(project1)
        expect(new_membership.user.nickname).to eq('test_final_user')
        expect(new_membership.role_cd).to eq(2) # member role
      end
