  ...
  context 'authenticated user' do
    ...
    context 'message processing' do
      it 'processes valid time tracking message without errors' do
        expect { dispatch_message('2.5 work-project testing feature') }.not_to raise_error
      end

      it 'processes reverse format message without errors' do
        expect { dispatch_message('work-project 1.5 bug fixing') }.not_to raise_error
      end

      it 'processes message without description without errors' do
        expect { dispatch_message('3 work-project') }.not_to raise_error
      end

      it 'processes decimal comma without errors' do
        expect { dispatch_message('1,5 work-project some work') }.not_to raise_error
      end

      it 'processes unicode characters without errors' do
        expect { dispatch_message('2.5 work-project разработка функции') }.not_to raise_error
      end
    end
    ...
    context 'message validation' do
      ...
      it 'handles message with too few parts without errors' do
        expect { dispatch_message('2.5') }.not_to raise_error
      end
    ...
    context 'error handling' do
      it 'handles non-existent project without errors' do
        expect { dispatch_message('2.5 nonexistent test') }.not_to raise_error
      end

      it 'handles invalid time format without errors' do
        expect { dispatch_message('abc work-project test') }.not_to raise_error
      end

      it 'handles too much time without errors' do
        expect { dispatch_message('25 work-project test') }.not_to raise_error
      end

      it 'handles too little time without errors' do
        expect { dispatch_message('0.05 work-project test') }.not_to raise_error
      end

      it 'handles negative time without errors' do
        expect { dispatch_message('-2 work-project test') }.not_to raise_error
      end
    end

    context 'TelegramTimeTracker integration' do
      it 'integrates with TelegramTimeTracker service' do
        expect(TelegramTimeTracker).to receive(:new).with(telegram_user, anything).and_call_original

        expect { dispatch_message('2.5 work-project testing') }.not_to raise_error
      end
