...
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name pythia pythia
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl libjemalloc2 libvips postgresql-client git ssh && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives

RUN mkdir /rails/.ssh
RUN ssh-keyscan -t rsa github.com >> /rails/.ssh/known_hosts
